...
sub useCWD {
    ...
    {                     #use current directory to set up repo
        git();
    } elsif ( $selectDir eq 'yes' && $menuSelect == 2 )
    {    #use current directory to add/push another file to existing repo
        addanother();
    } else {
...
sub checkLength {
    print(
"Add small message for the commit NO SPACES use an underscore, \nexample:\"added_file\".Use less than 30 chars\n"
    );
    $commitMssg = <STDIN>;
    chomp $commitMssg;
    $lengthcommitMssg =
      length($commitMssg);   #use built-in 'length' function to get No. of chars
    if ( $lengthcommitMssg > 30 ) {
        print("\nYour message is too long, please try a shorter one\n");
        checkLength();
    } else {
        system "git commit -m" . $commitMssg . ">>commit_record"
          || die '\nCannot add commit message: $!'
          ;                  #add commit message and append 'commit_record' file
        open( NFH, '>>commit_record' );
        if ( !print NFH $commitMssg,
            " ", scalar(localtime) . "\nEND OF LINE\n*\t*\t*\t*\t*\t*\n" )
        {                    #add timestamp
...
sub dirPath {
    ...
    chomp($dir);
    if ( opendir( DIR, $dir ) )
    {    #if dir path is ok then change to dir and list files
        chdir $dir;
        ...
        if ( $menuSelect == 2 ) {
            ...
            git()
              ; # run git() function to connect to git hub if user chose option 1
        }
...
sub git {
    ...
    system 'git config --global user.name' . $user
      || die "Cannot set user name: $!"
      ;                 #set global user name and email for git hub repo
    system 'git config --global user.email' . $email
      || die "Cannot set email: $!";
    ...
    if ( $selectReadme eq 'yes' ) {
        ...
        system "git remote add origin git\@github.com:" . $user . "/" . $repo
          . ".git";
